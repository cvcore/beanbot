...
def download_transactions(dkb_session: DKBRobo, account_lastfour: str, date_from: str, date_to: str, allow_prebooked: bool=False) -> List:
    ...
    assert link_transactions is not None, f"Account with last four digits {account_lastfour} not found."
    pprint(link_transactions)

    transactions = dkb_session.get_transactions(link_transactions, "account", date_from, date_to, 'booked')
    for txn in transactions:
        txn['status'] = 'booked'
    if allow_prebooked:
        reserved_txns = dkb_session.get_transactions(link_transactions, "account", date_from, date_to, 'reserved')
...
def save_as_csv(transactions: List, filename: str, iban: str, account_type: str, start_date: str, end_date: str, balance: str):
...
def download_all_csv_files(account_info: List, dkb_session: DKBRobo):
    ...
    for account in account_info:
        ...
        print(f"[INFO] Downloading transactions for {account_type} ({account_lastfour})...")
